...
class Router
{
    ...
    private function dispatchHandler(callable|array|string $callback, array $params): void
    {
        ...
        // Gestion intelligente de la pluralisation (ex: Stand -> stands, Category -> categories, Auth -> auths/auth)
        ...
        // Résolution dynamique avec support explicite des dossiers singuliers/pluriels et du module auth
        $possiblePaths = [
        ...
        // Si toujours non trouvé, recherche globale récursive dans le dossier modules/
        if (!$fileFound) {
            foreach (glob($baseAppPath . '/modules/*/controllers/' . $className . '.php') as $globFile) {
                if (file_exists($globFile)) {
                    require_once $globFile;
                    $fileFound = true;
                    break;
                }
            }
        ...
        // Détermination du Nom de Classe Totalement Qualifié (FQCN) avec Namespace
        $fullyQualifiedClass = $className;
        if (str_starts_with($className, 'App\\Modules\\')) {
            $fullyQualifiedClass = $className;
        } else {
            // Tente de reconstruire le namespace standard des modules
            $candidateFqcn = "App\\Modules\\" . ucfirst($rawModuleName) . "\\Controllers\\" . $className;
            if (class_exists($candidateFqcn)) {
                $fullyQualifiedClass = $candidateFqcn;
            } else {
                // Recherche dans tous les sous-dossiers de modules si la classe existe avec le namespace
                foreach (['Auth', 'Kyc', 'Stand', 'Product', 'Admin', 'User'] as $modTest) {
                    $testFqcn = "App\\Modules\\" . $modTest . "\\Controllers\\" . $className;
                    if (class_exists($testFqcn)) {
                        $fullyQualifiedClass = $testFqcn;
                        break;
                    }
                }
            }
        ...
        if (!class_exists($fullyQualifiedClass) && !class_exists($className)) {
            $this->response->error("Contrôleur '{$className}' introuvable.", 500);
            return;
        }

        $targetClass = class_exists($fullyQualifiedClass) ? $fullyQualifiedClass : $className;
        ...
        try {
            ...
            if ($constructor !== null && $constructor->getNumberOfParameters() > 0) {
                $controller = new $targetClass($this->request);
            } else {
                $controller = new $targetClass();
            }
        } catch (Throwable $e) {
            $this->response->error("Erreur lors de l'instanciation de '{$targetClass}' : " . $e->getMessage(), 500);
            return;
        }

        if (!method_exists($controller, $methodName)) {
            $this->response->error("Méthode '{$methodName}' introuvable dans le contrôleur '{$targetClass}'.", 500);
            return;
        }
        ...
        try {
            ...
            $this->response->error("Erreur lors de l'exécution de {$targetClass}@{$methodName} : " . $e->getMessage(), 500);
        }
    }
