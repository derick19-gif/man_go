...
<!-- Ajout du CSS pour l'animation juste APRÈS la fermeture de la balise PHP -->
<style>
@keyframes marquee {
...
.animate-marquee {
  /* Vous pouvez changer 25s par un chiffre plus petit pour aller plus vite */
...
<div class="max-w-5xl mx-auto relative z-10 pt-20">
    ...
    <form action="<?= $baseUrl ?>/" method="GET" class="mt-8 bg-white/10 backdrop-blur-xl p-3 rounded-2xl sm:rounded-full border border-white/20 shadow-2xl flex flex-col sm:flex-row gap-2 max-w-4xl mx-auto mb-20">
        <?php if(!empty($lang)): ?>
...
<!-- Section Exploration par Catégories (Animée) -->
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full relative z-20">
    ...
    <div class="relative overflow-hidden w-full group py-4">
        <div class="flex space-x-5 animate-marquee group-hover:pause">
            <?php if (!empty($categories)): ?>
                <!-- Boucle de vos catégories (dupliquée pour l'effet infini) -->
                <?php for ($i = 0; $i < 2; $i++): ?>
                    <?php foreach ($categories as $cat): ?>
                        <?php $icon = $getCategoryIcon($cat['name'] ?? '', $iconMap); ?>
                        <a href="<?= $baseUrl ?>/listings?category=<?= (int)($cat['id'] ?? 0) ?>" class="flex-shrink-0 w-48 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:border-amber-400 transition-all duration-300 transform hover:-translate-y-2 flex flex-col items-center text-center">
                ...
                <?php endfor; ?>
            <?php else: ?>
                <!-- Vos Fallbacks (dupliqués aussi) -->
                <?php for ($i = 0; $i < 2; $i++): ?>
                    <a href="<?= $baseUrl ?>/listings" class="flex-shrink-0 w-48 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-2 transition flex flex-col items-center text-center">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        <?php if (!empty($listings)): ?>
            <?php foreach ($listings as $item): ?>
                ...
                ?>
                <a href="<?= $baseUrl ?>/listing_detail?id=<?= (int)($item['id'] ?? 0) ?>" class="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group transform hover:-translate-y-2">
